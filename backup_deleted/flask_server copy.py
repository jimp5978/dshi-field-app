...
def login():
    ...
    try:
        ...
        # 실제 MySQL 데이터베이스 연결 시도
        connection = get_db_connection()
        if not connection:
            print("DB 연결 실패 - 다양한 방법 시도")
            # 다양한 연결 방법 시도
            connection_attempts = [
                {"host": "localhost", "port": 3306, "user": "field_app_user", "password": ""},
                {"host": "127.0.0.1", "port": 3306, "user": "field_app_user", "password": ""},
                {"host": "localhost", "port": 3307, "user": "field_app_user", "password": ""},
                {"host": "localhost", "port": 3306, "user": "root", "password": ""},
                {"host": "localhost", "port": 3306, "user": "root", "password": "dshi_root_2024"},
            ]
            ...
            for attempt in connection_attempts:
                try:
                    ...
                    print(f"연결 성공: {attempt['user']}@{attempt['host']}:{attempt['port']}")
                    break
                except Exception as e:
                    print(f"연결 실패 {attempt['user']}@{attempt['host']}:{attempt['port']}: {e}")
                    continue
        
        if not connection:
            return jsonify({'success': False, 'message': '데이터베이스에 연결할 수 없습니다'}), 500
        ...
        try:
            ...
            # 데이터베이스에서 사용자 조회
            cursor.execute("""
                SELECT id, username, password_hash, full_name, permission_level, 
                       COALESCE(company, '') as company, is_active
                FROM users 
    ...
    try:
        ...
        for row in rows:
            ...
            processes = [
                ...
                ('ARUP_FINAL', row['arup_final_date']),
                ('GALV', row['galv_date']),
                ('ARUP_GALV', row['arup_galv_date']),
                ('SHOT', row['shot_date']),
                ('PAINT', row['paint_date']),
                ('ARUP_PAINT', row['arup_paint_date'])
            ]
            ...
            assemblies.append({
                ...
                'weight_net': float(row['weight_net']) if row['weight_net'] else 0.0,
...
def reject_inspection_request(current_user, request_id):
    ...
    try:
        ...
        if not inspection_request:
            return jsonify({'success': False, 'message': '거부할 수 있는 검사신청이 없습니다'}), 404
        ...
        cursor.execute("""
            ...
            SET status = '거부됨',
                ...
                reject_reason = %s
            WHERE id = %s
        """, (current_user, user_info['full_name'], today, reject_reason, request_id))
        ...
        return jsonify({
            'success': True,
            'message': f'{inspection_request["assembly_code"]} {inspection_request["inspection_type"]} 검사신청이 거부되었습니다'
        })
...
def delete_inspection_request(current_user, request_id):
    ...
    try:
        ...
        # Level 3+는 모든 상태 삭제 가능
        
        # 트랜잭션 시작
        try:
            # 확정됨 상태에서 삭제하는 경우 arup_ecs 테이블 롤백 필요
            if inspection_request['status'] == '확정됨':
                ...
                if inspection_type in inspection_type_mapping:
                    ...
                    # arup_ecs 테이블의 해당 공정 날짜를 NULL로 되돌리기
                    update_query = f"""
                        UPDATE arup_ecs 
            ...
            # 삭제된 행이 있는지 확인
            if cursor.rowcount == 0:
                raise Exception('삭제할 검사신청을 찾을 수 없습니다')
        ...
        # 성공 메시지 생성
        message = f'{inspection_request["assembly_code"]} {inspection_request["inspection_type"]} 검사신청이 삭제되었습니다'
        
        # 확정된 항목이 삭제된 경우 추가 정보 제공
        if inspection_request['status'] == '확정됨':
